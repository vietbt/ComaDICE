...
from agents.continuous import ICQ
...
class Trainer:
    ...
    def __init__(self, logdir, dataset, n_agents, lr, seed, device):
        ...
        self.agent = ICQ(st_dim, ac_dim, n_agents, self.eval_env, config)
    ...
    def train(self, n_epochs, n_evals=100, batch_size=128):
        ...
        while True:
            states, obs, actions, rewards, masks, next_states, next_obs, next_actions = self.offline_data.sample(batch_size, self.device, use_next_action=True)
            losses = self.agent.train_step(obs, states, actions, rewards, masks, next_states, next_obs, next_actions)
            self.global_step += 1
